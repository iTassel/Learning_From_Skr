...
bool handle_write(ClientCtx& client) {
  for (;;) {
    ...
    int written = write(client.fd, &client.wr_buf[0], client.wr_buf.size()); // print the string when the size is wi_buf.size()
    if (written < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        break;
      }
    ...
    if (written > 0) {
      client.wr_buf = client.wr_buf.substr(written); // cut the rest string from the wr_buf
    }
  }
  return true;
}
