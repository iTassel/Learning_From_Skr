...
put_kstack(struct proc_struct *proc) {
    free_pages(kva2page((void *)(proc->kstack)), KSTACKPAGE); //释放proc进程的堆栈空间
}

// setup_pgdir - 申请一个内存页作为新进程的页目录表
static int
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    if (copy_mm(clone_flags, proc) != 0) { //设置mm指针的指向
 ...
 */
static int
load_icode(unsigned char *binary, size_t size) { //创建一个用户环境
    if (current->mm != NULL) {
...
init_main(void *arg) {
    ...
    int pid = kernel_thread(user_main, NULL, 0); //进程参数设置
    if (pid <= 0) {
...
proc_init(void) {
    ...
    int pid = kernel_thread(init_main, NULL, 0); // 进程参数设置
    if (pid <= 0) {
